...
class ListCommandsTests: XCTestCase {
    ...
    struct CommandStruct {
        ...
        func getCommandDict() -> [String: AnyObject] {
            var dict: [String: AnyObject] = ["commandID": commandID, "schema": schema, "schemaVersion": schemaVersion, "target": "\(target.targetType.type):\(target.targetType.typedID)", "commandState": commandStateString, "issuer": "\(issuerID.type):\(issuerID.typedID)", "actions": actions]
    ...
    func testListCommand_404_error() {
        ...
        do{
            ...
            let requestVerifier: ((NSURLRequest) -> Void) = {(request) in
                XCTAssertEqual(request.HTTPMethod, "GET")
                // verify path
                let expectedPath = "\(self.api.baseURL!)/iot-api/apps/\(self.api.appID!)/targets/\(self.target.targetType.type):\(self.target.targetType.typedID)/commands"
