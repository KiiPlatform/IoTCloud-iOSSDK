...
class GetCommandTests: XCTestCase {
    ...
    let owner = Owner(ownerID: TypedID(type:"user", typedID:"53ae324be5a0-2b09-5e11-6cc3-0862359e"), accessToken: "BbBFQMkOlEI9G1RZrb2Elmsu5ux1h-TIm5CGgh9UBMc")
    ...
    let target = Target(targetType: TypedID(type: "thing", typedID: "th.0267251d9d60-1858-5e11-3dc3-00f3f0b5"))

    override func setUp() {
        super.setUp()
        api = IoTCloudAPIBuilder(appID: "50a62843", appKey: "2bde7d4e3eed1ad62c306dd2144bb2b0",
            site: Site.CUSTOM("https://api-development-jp.internal.kii.com"), owner: Owner(ownerID: TypedID(type:"user", typedID:"53ae324be5a0-2b09-5e11-6cc3-0862359e"), accessToken: "BbBFQMkOlEI9G1RZrb2Elmsu5ux1h-TIm5CGgh9UBMc")).build()
    }
    ...
    func testGetCommandSuccess() {
        ...
        let testcases = [
            TestCase(target: self.target, schema: self.schema.name, schemaVersion: self.schema.version, actions: [["turnPower":["power": true]]], issuerIDString: "\(self.owner.ownerID.type):\(self.owner.ownerID.typedID)", targetIDString: "\(self.target.targetType.type):\(self.target.targetType.typedID)", actionResults: [["turnPower":["power": true]]], commandState: CommandState.INCOMPLETE, commandStateString: "INCOMPLETE"),
            TestCase(target: self.target, schema: self.schema.name, schemaVersion: self.schema.version, actions: [["setBrightness":["brightness": 100]]],  issuerIDString: "\(self.owner.ownerID.type):\(self.owner.ownerID.typedID)", targetIDString: "\(self.target.targetType.type):\(self.target.targetType.typedID)", actionResults:nil, commandState: CommandState.SENDING, commandStateString: "SENDING"),
            ...
            TestCase(target: self.target, schema: self.schema.name, schemaVersion: self.schema.version, actions: [["turnPower": ["power": true]], ["setBrightness": ["brightness": 100]]],  issuerIDString: "\(self.owner.ownerID.type):\(self.owner.ownerID.typedID)", targetIDString: "\(self.target.targetType.type):\(self.target.targetType.typedID)", actionResults:[["turnPower": ["power": true]], ["setBrightness": ["brightness": 100]]], commandState: CommandState.DONE, commandStateString: "DONE")
    ...
    func testGetCommand_404_error() {
        ...
        do{
            ...
            let requestVerifier: ((NSURLRequest) -> Void) = {(request) in
                XCTAssertEqual(request.HTTPMethod, "GET")
                // verify path
                let expectedPath = "\(self.api.baseURL!)/iot-api/apps/\(self.api.appID!)/targets/\(self.target.targetType.type):\(self.target.targetType.typedID)/commands/\(commandID)"
